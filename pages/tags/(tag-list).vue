...
<template>
  <div class="py-12">
    ...
    <div v-if="error" class="animate text-[var(--color-error)] mb-8">
      {{ error.message }}
    </div>

    <div v-else-if="loading" class="animate text-[var(--color-text-muted)]">
      Loading tags...
    </div>
    ...
    <div v-else class="animate text-[var(--color-text-muted)] py-8 text-center">
      No tags found.
    </div>
  </div>
</template>
